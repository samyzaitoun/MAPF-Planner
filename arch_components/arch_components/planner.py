...
class Planner(Node):
    ...
    def __init__(self):
        ...
        self.plan_srv = self.create_service(PlanRequest, 'plan_request', self.plan_callback)
    ...
    def get_launch_parameters(self) -> None:
        ...
        self.arena_frame: str = self.get_parameter(
            'tf_tag_arena').get_parameter_value().string_value
        
        ## Arena height (int)
        self.declare_parameter('arena_height')
        self.arena_height: int = self.get_parameter(
            'arena_height').get_parameter_value().integer_value

        ## Arena width (int)
        self.declare_parameter('arena_width')
        self.arena_width: int = self.get_parameter(
            'arena_width').get_parameter_value().integer_value
        
        ## Agent diameter (int)
        self.declare_parameter('agent_diameter')
        self.agent_diameter: int = self.get_parameter(
            'agent_diameter').get_parameter_value().integer_value
        ...
        self.mapf_solver_class: Type[MAPFSolver] = self.get_parameter(
            'mapf_solver').get_parameter_value() # Not Defined, should work..?
        ...
        self.mapf_input_class: Type[MAPFInput] = self.get_parameter(
            'mapf_input').get_parameter_value() # Not Defined, should work..?
        
        ## MAPF Algorithm Time limit (float)
        self.declare_parameter('time_limit', NO_TIME_LIMIT)
        self.time_limit: float = self.get_parameter(
            'time_limit').get_parameter_value().double_value
    ...
    def generate_and_solve_map(
        ...
        discrete_agents = [
            WayPoint(x=agent.x/self.agent_diameter, y=agent.z/self.agent_diameter) 
            for agent in agents
        ]
        # Algo lib uses TimedWayPoint for goals and I don't understand why. (It doesn't supply a t value...)
        discrete_goals = [
            TimedWayPoint(x=goal[1].x/self.agent_diameter, y=goal[1].y/self.agent_diameter) 
            for goal in goals
        ]
        discrete_obstacles = [
            (obstacle.x/self.agent_diameter, obstacle.z/self.agent_diameter) 
